---
report_id: 20260701-openai-agents-sdk-v1.1
title: OpenAI Agents SDK 技术分析报告 v1.1
version: v1.1
...
OpenAI Agents SDK 是 OpenAI 官方推出的轻量级多 Agent 工作流构建框架，于 2025 年 3 月首次发布，2026 年 4 月 15 日发布重大更新，正式从"实验工具"进化为"生产级 AI Agent 基础设施"。<!-- confidence: EXTRACTED -->

该框架的核心设计哲学是**极简主义**：足够的抽象，不过度封装，几十行代码就能跑起一个有工具调用能力的 Agent。它通过 LiteLLM/any-llm 兼容层支持 100+ 非 OpenAI 模型，原生集成 MCP 协议，提供 Handoff 多 Agent 协作、Guardrails 安全护栏、Sandbox 沙箱执行、Tracing 可观测性等企业级能力。<!-- confidence: INFERRED -->

截至 2026 年 6 月，最新版本为 v0.17.7（Python）和 v0.12.0（JS/TS），GitHub 仓库拥有超过 20k Stars（估算值，2026年4月数据为21,925），是 2026 年最受关注的 AI Agent 框架之一。<!-- confidence: INFERRED -->
...
| **Handoff** | Agent 间任务移交 | LLM 自动路由（社区分析）、上下文传递、状态切换 |
...
| **Sandbox** | 隔离执行环境（beta） | 文件系统、shell、包管理、快照、Manifest 权限控制 |
...
**核心 Agent 能力**：
- ✅ 多模型支持（通过 LiteLLM/any-llm 支持 100+ LLM，OpenAI 优先）
- ✅ 函数调用（Function Tools）
- ✅ 托管工具（web search、file search、code interpreter 等，OpenAI 模型）
- ✅ MCP 协议原生支持
- ✅ 结构化输出（Output Type）
- ✅ 自定义 Hooks
...
- ✅ Session 会话持久化（多种后端）
- ✅ Tracing 运行追踪（默认启用）
- ✅ Usage 统计（token 消耗统计，第三方模型可能不完整）
- ✅ Eval 评估框架
- ✅ 流式事件输出
- ✅ 上下文压缩（Compaction）
...
**模型绑定**：
- 虽然通过兼容层支持 100+ 非 OpenAI 模型，但 OpenAI 模型体验最优
...
> 注：Microsoft AutoGen 已于 2025 年 10 月进入维护模式，微软将重心转向 Microsoft Agent Framework (MAF)。<!-- confidence: EXTRACTED -->
...
| 编号 | 声明 | 优先级 | 验证方式 | 状态 |
|------|------|--------|----------|------|
| 1 | GitHub Stars 超过 20k（估算） | P1 | 官方 GitHub 页面验证 | ⚠️ 部分验证 |
| 2 | 通过 LiteLLM/any-llm 支持 100+ 非 OpenAI 模型 | P1 | 官方文档验证 | ✅ 基本验证 |
| 3 | 2026 年 4 月 15 日发布重大更新 | P1 | 官方博客/Release Notes 验证 | ✅ 已验证 |
| 4 | Sandbox Agents 处于 beta 阶段 | P2 | 官方文档验证 | ✅ 已验证 |
| 5 | AutoGen 已进入维护模式 | P2 | 微软官方公告验证 | ✅ 已验证 |
| 6 | Handoff 路由由 LLM 自动判断 | P2 | 官方文档/代码验证 | ⚠️ 待深入验证 |
...
| v1.1 | 2026-07-01 | 证伪修正：调整 Stars 数量表述、补充多模型支持实现方式、更新验证状态、AutoGen 维护模式加注释说明 |
