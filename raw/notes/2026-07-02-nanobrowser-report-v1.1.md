---
report_id: 20260702-nanobrowser-1.1
title: Nanobrowser 技术分析报告 v1.1
version: 1.1
...
Nanobrowser 是一款开源的 AI 网页自动化工具，以 Chrome 浏览器扩展形式运行，提供多智能体协作系统，是 OpenAI Operator 的免费替代方案。其核心创新在于将 AI Agent 能力直接集成到浏览器中，通过 Planner（规划器）和 Navigator（导航者）双智能体协作实现复杂网页任务自动化，支持多种 LLM 提供商。操作逻辑在浏览器本地执行，但 LLM 调用仍需通过外部 API 传输数据。
...
- **浏览器内执行**：操作逻辑在浏览器本地完成，无需外部服务器 <!-- confidence: EXTRACTED -->

### 3.2 局限性

- **浏览器限制**：仅支持 Chrome 和 Edge，不支持 Firefox、Safari <!-- confidence: EXTRACTED -->
- **数据传输**：LLM 调用需通过外部 API，数据会离开本地浏览器 <!-- confidence: INFERRED -->
...
- **复杂场景**：需要验证码、多步骤登录
- **跨浏览器**：需要多浏览器兼容
- **严格隐私要求**：需要数据完全不离开本地的场景（LLM调用会传输数据）
...
| v1.1 | 2026-07-02 | 修正隐私保护声明：明确LLM调用会传输数据到外部API |