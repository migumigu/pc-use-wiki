...
title: 系统服务控制 - 硬件资源监控技术分析报告 v1.1
version: 1.1
...
- netifaces 提供跨平台网络接口信息获取能力，但项目已停止维护（最后更新 2021 年）
...
**GPUtil：**
- 仅支持 NVIDIA GPU（依赖 nvidia-smi）<!-- confidence: EXTRACTED -->
- 不支持 AMD/Intel GPU
- 不支持 GPU 性能指标的实时监控
- 功能较单一，仅专注于 GPU 可用性检测

**netifaces：**
- ⚠️ 项目已停止维护（最后更新 2021 年）<!-- confidence: EXTRACTED -->
- ⚠️ Python 3.10+ 兼容性未明确验证<!-- confidence: UNVERIFIED -->
- 需要编译 C 扩展，安装复杂度较高
- 不支持网络流量监控
- 不支持接口状态实时变更监听
...
- ⚠️ Apple Silicon 支持未明确声明<!-- confidence: UNVERIFIED -->
- 部分架构支持有限（ARM/MIPS 等）<!-- confidence: INFERRED -->
- 不支持 CPU 温度监控
...
| 工具 | 问题 | 来源 | 置信度 |
|------|------|------|--------|
| GPUtil | 在某些驱动版本下可能返回 NaN 值 | GitHub Issues | EXTRACTED |
| GPUtil | GPU ID 可能在重启后变化 | README | EXTRACTED |
| netifaces | Windows Python 3.9+ 支持问题 | GitHub Issues | EXTRACTED |
| netifaces | 缺少维护者 | README 警告 | EXTRACTED |
| py-cpuinfo | ARM 架构信息不完整 | README | EXTRACTED |
...
| v1.1 | 2026-06-28 | 添加置信度标记，修正待验证声明 |