---
report_id: 2026-06-28-file-system-report-v1.1
title: 文件系统控制技术栈深度分析报告 v1.1
version: 1.1
...
**核心发现**：
1. **文件同步工具分化明显**：Syncthing（73,117 Stars，修正后）采用 P2P 无服务器架构，强调安全和自动化；Unison（25+ 年历史）专注双向同步和精确规范 <!-- confidence: EXTRACTED -->
2. **跨平台锁机制成熟**：filelock（723K+ 使用）提供 5 种锁类型，覆盖 Windows/Unix/网络文件系统，自动回退机制确保可移植性 <!-- confidence: EXTRACTED -->
3. **Python stdlib 局限性**：shutil 无法复制完整元数据（owner/group/ACLs），Agent 集成需注意权限丢失风险 <!-- confidence: EXTRACTED -->
...
| **Stars/使用数** | 73,117 Stars（修正） <!-- confidence: EXTRACTED --> | 25+ 年使用 <!-- confidence: EXTRACTED --> | 723K+ 使用 <!-- confidence: EXTRACTED --> | stdlib |
...
- 复制、移动、删除、权限操作
- 警告：无法复制完整元数据 <!-- confidence: EXTRACTED -->
...
**局限性** <!-- confidence: EXTRACTED -->：
- ❌ 无法复制完整元数据
  - POSIX: owner/group/ACLs 丢失 <!-- confidence: EXTRACTED -->
  - Mac OS: resource fork 丢失 <!-- confidence: EXTRACTED -->
  - Windows: owner/ACLs/备用数据流丢失 <!-- confidence: EXTRACTED -->
...
| 来源 | 类型 | 置信度 | 主要贡献 |
|------|------|--------|----------|
| [[syncthing-github-readme]] | Tier 1 | EXTRACTED | 核心目标、架构、Device ID |
...
| GitHub API | Tier 1 | EXTRACTED | Syncthing Stars 修正（73,117） |
...
1. **Syncthing Stars 确切值** ✅已验证
   - 声明：62.2K+ Stars → 修正为：73,117 Stars <!-- confidence: EXTRACTED -->
   - 来源：GitHub API (https://api.github.com/repos/syncthing/syncthing)

2. **shutil 元数据丢失的实际影响** ✅已验证
   - 声明：POSIX/Mac/Windows 都丢失元数据 <!-- confidence: EXTRACTED -->
   - 来源：Python 官方文档

### P2 中优先级（待补充）

3. **filelock 使用数确切值** ⚠️待验证
   - 声明：723K+ 使用数 <!-- confidence: EXTRACTED -->
   - 来源：GitHub README
   - 状态：无法访问 dependents 页面（需登录态）
...
| v1.1 | 2026-06-28 | 修正 Syncthing Stars 数据（62.2K+ → 73,117），基于 GitHub API |
...
- GitHub API (https://api.github.com/repos/syncthing/syncthing) - Stars 数据修正