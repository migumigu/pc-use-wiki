---
report_id: 2026-06-30-turix-cua-report-v1.0
title: TuriX-CUA 技术分析报告 v1.0
version: v1.0
...
**关键数据**：
- OSWorld Benchmark 第3名（64.2% 成功率）
- macOS 专用基准测试 80%+ 成功率
...
| 能力 | 描述 | 证据来源 |
|------|------|----------|
| **全应用操作** | 无需特定 API，只要人能点的都能操作 | README |
| **屏幕理解** | 视觉语言模型理解当前界面状态 | README |
| **多平台支持** | macOS/Windows/Linux | README |
| **浏览器自动化** | Safari/Chrome 等主流浏览器 | 技术分析 |
| **MCP 集成** | 可与 Claude Desktop 等协作 | README |
| **Skills 系统** | 可定制的重复任务剧本 | README |
| **任务恢复** | 中断后可继续执行 | README |
| **本地模型** | 支持 Ollama 本地部署 | README |

### 3.2 性能指标

| 基准 | 结果 | 排名 |
|------|------|------|
| OSWorld Benchmark（50步） | 64.2%（229.88/358） | 第3名 |
| macOS 内部基准 | 80%+ 成功率 | N/A |
| 训练数据 | 零 Linux 训练数据 | N/A |

### 3.3 局限性

| 局限性 | 描述 | 严重程度 |
|--------|------|----------|
| **需要 API 密钥** | 官方 API 或本地 Ollama | 中 |
| **权限依赖** | macOS 需要辅助功能权限 | 中 |
| **视觉模型局限** | 复杂 UI 识别可能出错 | 低 |
| **长任务稳定性** | 50+ 步任务成功率下降 | 中 |
| **模型成本** | 多模型调用成本较高 | 低 |
...
2. **macOS 80%+ 成功率**：这是内部基准测试，缺乏第三方验证
