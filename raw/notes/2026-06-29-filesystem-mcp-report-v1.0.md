...
Filesystem MCP Server 是 Anthropic 官方维护的 Model Context Protocol 参考实现之一，为 AI Agent 提供安全、受控的文件系统操作能力。作为 MCP 官方服务器的七个参考实现之一，Filesystem MCP 已成为 AI Agent 文件系统交互的事实标准，被描述为"存在之钉"（Existence Spike）——所有高阶 MCP 服务的基础依赖。截至2026年3月，安装量已达48.5万次，Token效率约800 tokens。
...
基于 GitHub Issues 和社区反馈：

1. **路径验证漏洞**：某些边界情况下的路径验证不完善
2. **Windows 兼容性**：反斜杠路径处理存在问题
3. **符号链接穿透**：可能通过符号链接访问白名单外目录
4. **大文件处理**：对超大文件（>100MB）可能 OOM
...
| [[auto-2026-06-29-filesystem-mcp-analysis]] | Tier 2 | INFERRED | 深度分析、安全评估 |

## 7. 待验证问题

| 问题 | 声明来源 | 验证优先级 | 状态 |
|------|----------|------------|------|
| 48.5万安装量 | 博客文章 | P2 | ⚠️ 待核实 |
| 800 tokens 上下文成本 | 博客文章 | P3 | ⚠️ 待核实 |
| 符号链接安全漏洞 | 技术分析 | P2 | ⚠️ 待核实 |
| Windows 路径问题 | GitHub Issues | P1 | ✅ 已知问题 |
