---
report_id: auto-2026-06-28-agent-browser-report-v1.1
...
# agent-browser 技术分析报告 v1.1

> 生成日期：2026-06-28
> 来源：5 个（Tier1: 3, Tier2: 2）
> 报告版本：v1.1（已证伪修正）
> 证伪记录：见 [[2026-06-28-agent-browser-falsification]]

## 1. 执行摘要

**agent-browser** 是由 Vercel Labs 于 2026 年 1 月开源的 AI 浏览器自动化 CLI 工具，专为 AI Agent 设计。截至 2026-06-28 已获得 **37,353 Stars**、2,398 Forks，成为浏览器自动化领域的现象级项目。

核心创新：
- **refs 机制**：用 @e1、@e2 等确定性引用替代 CSS 选择器，解决 AI"看不清、点不准"的痛点
- **Rust 原生 CLI**：性能显著提升（官方未提供 benchmark 数据）
- **极低 Token 消耗**：第三方测试显示 Token 消耗降低最高可达 93%
...
| **snapshot** | 无障碍树快照 | 只返回交互元素，Token 消耗大幅降低 |
...
| 限制 | 说明 | 置信度 |
|------|------|--------|
| **相对年轻** | 项目仅开源约 6 个月，部分边缘功能仍在完善 | EXTRACTED |
| **Windows 支持** | 早期版本有路径问题，后续版本已改善 | INFERRED |
| **浏览器支持** | 主要支持 Chrome，Firefox/Safari 仍在开发 | EXTRACTED |
| **功能相对简化** | 相比 Playwright 功能较少 | EXTRACTED |

### 3.3 已知问题

来自 GitHub Issues 和社区反馈：
- open_issues_count: 542（截至 2026-06-28）
...
| **性能** | 显著提升 | 中等 | 较慢 | 中等 |
| **Token 消耗** | 大幅降低 | 高 | 极高 | 高 |
| **CDP 原生** | Yes | via Playwright | via Playwright | Yes |
| **CLI 一致性** | Yes | No | Via bindings | Via CLI |
| **会话隔离** | Yes | 有限 | 有限 | 有限 |
| **Stars** | 37,353 | 已有 | N/A | 已有 |

### 4.2 Token 效率对比

> ⚠️ 以下数据来自第三方测试（Token 对比文章），置信度为 INFERRED
...
| 指标 | 数值 | 验证状态 |
|------|------|----------|
| **Stars** | 37,353 | ✅ GitHub API |
| **Forks** | 2,398 | ✅ GitHub API |
| **Open Issues** | 542 | ✅ GitHub API |
| **Commits** | 576 | ✅ GitHub README |
| **Branches** | 286 | ✅ GitHub README |
| **Tags** | 82 | ✅ GitHub README |
| **创建日期** | 2026-01-11 | ✅ GitHub API |
| **语言** | Rust | ✅ GitHub API |
...
| 问题类型 | 数量 | 说明 |
|----------|------|------|
| ⚠️ 数据不准确 | 2 | Stars 实际更高、项目年龄更长 |
| ❌ 未经核实声明 | 1 | "5-10倍性能提升"无官方依据 |
| ⚠️ 待验证声明 | 2 | Token 效率数据、版本号 |
...
|------|------|----------|
| v1 | 2026-06-28 | 初始版本 |
| v1.1 | 2026-06-28 | 证伪修正：更新 Stars 数据、修正项目年龄、删除未核实性能声明、添加项目统计 |

## 10. 下一步研究建议

1. **实际性能测试**：运行 benchmark 对比 Playwright/Puppeteer
2. **Windows 环境测试**：验证最新版本的 Windows 支持
3. **架构源码分析**：深入研究 Rust CLI 和 daemon 实现
4. **与其他工具对比**：browser-use、Playwright MCP 详细对比
5. **GitHub Issues 分析**：分析 542 个 open issues 了解项目痛点
