---
report_id: 20260629-bytebot-v1.0
title: Bytebot 容器化虚拟桌面 Computer Use Agent 技术分析报告 v1.0
version: v1.0
...
# Bytebot 容器化虚拟桌面 Computer Use Agent 技术分析报告 v1.0
...
- **MCP 协议原生支持**：可作为 MCP Server 接入 AI 生态
- **API 驱动**：提供 REST API 和 WebSocket，支持程序化控制

**项目状态**：GitHub 仓库已归档（2025年9月最后提交），但架构设计完整，对研究容器化 Computer Use 方案仍有重要参考价值。
...
**工具实现层**：
- bytebotd 守护进程：基于 nutjs 框架的自动化执行
- Prisma ORM：数据库操作
- LiteLLM 集成：多模型支持
...
| **核心技术** | nutjs + LLM | VLM + 轨迹预测 | VLM + GUI Agent | MCP + Skills |
| **MCP 支持** | 原生支持 | 支持 | MCP 工具链 | 原生支持 |
| **项目状态** | 已归档 | 活跃 | 活跃 | 活跃 |
...
| [[auto-20260629-bytebot-rpa]] | Tier 1 | EXTRACTED | 与传统 RPA 对比、企业应用场景 |
| [[auto-20260629-bytebot-api]] | Tier 1 | EXTRACTED | API 接口、认证、错误处理 |

## 6. 待验证问题

1. **P1**：Bytebot 是否真的支持 MCP 协议？具体支持哪些工具？
2. **P1**：nutjs 自动化框架的能力边界是什么？支持哪些操作系统？
3. **P1**：项目是否真的完全停止开发了？是否有 fork 或社区维护版本？
4. **P2**：Bytebot 的任务执行成功率如何？有哪些类型任务表现最好？
5. **P2**：与其他 Computer Use Agent 相比性能对比数据如何？
6. **P3**：企业用户实际使用案例有哪些？
