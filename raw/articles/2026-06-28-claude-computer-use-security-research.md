...
- **来源层级**: Tier 1 - Anthropic官方安全研究
- **核心议题**: Browser Use场景下的Prompt Injection防御机制
- **风险特征**: Browser Use放大了Prompt Injection风险(攻击面广、可执行动作多)
- **防御措施**: 
  - 强化学习训练模型识别和拒绝恶意指令
  - Constitutional classifiers检测隐藏文本、图片注入、UI欺骗等
  - 专家红队测试和外部竞技场挑战
- **性能数据**: Claude Opus 4.5的攻击成功率约1%(原始版本为11%缓解前为23.6%)
- **重要声明**: 1%成功率仍代表重大风险,问题未完全解决
- **适用范围**: Claude for Chrome扩展从研究预览升级为Beta版