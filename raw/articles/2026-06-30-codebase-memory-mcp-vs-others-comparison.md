---
source_id: auto-20260630-cbm3
title: 编程 Agent 的地图之争 - codebase-memory-mcp、codegraph、codescope 对比
url: http://m.toutiao.com/group/7653740056691081734/
...
> codebase-memory-mcp、codegraph、codescope 三款代码智能工具深度对比
...
| **codebase-memory-mcp** | DeusData | 4K-17K* | C | MCP 代码智能服务器 |

*注：codebase-memory-mcp 的 Star 数数据来源不一致，从 4K 到 17K 都有报道，可能是不同时间点的数据。

### 架构对比

| 维度 | GitNexus | codegraph | codebase-memory-mcp |
|------|----------|-----------|---------------------|
| 核心语言 | Rust | TypeScript | C |
...
|------|----------|-----------|---------------------|
| 索引速度（10万行） | ~30s | ~60s | < 10s |
| 查询延迟 | ~50ms | ~100ms | < 1ms |
| Linux 内核索引 | 支持（~10min） | 部分支持（>30min） | 3 分钟 |
| 内存占用（中型项目） | ~500MB | ~300MB | ~50MB |
| Token 节省 | ~50 倍 | ~30 倍 | ~120 倍 |
