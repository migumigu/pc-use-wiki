...
The response contains a `tools` array that provides comprehensive metadata about each available tool. Each tool object in the response includes several key fields:
...
- **`inputSchema`**: A JSON Schema that defines the expected input parameters.
